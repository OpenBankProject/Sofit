...
package code.widgets.tableSorter {
...
object CustomTableSorter {
  ...
  def apply(selector: String): NodeSeq = renderOnLoad(selector, CustomTableSorter.options())
  def apply(selector: String, options: String): NodeSeq = renderOnLoad(selector, options)
  ...
  */
  def options(headers: List[(Int, TableSorterOption[_])], 
              widgets: List[String], 
              sortList: List[(Int, Sorting)]): String = {
  ...
  def options(): String = options(Nil, List("zebra"), List((0, ASC)))
  
  def options(headers: List[(Int, TableSorterOption[_])], 
              sortList: List[(Int, Sorting)]): String = options(headers, List("zebra"), sortList)
  ...
  */
  def renderOnLoad(selector: String, options: String): NodeSeq = {
   ...
   */
  def jsRender(selector: String, options: String): JsExp = 
    JqId(selector) ~> new JsRaw("tablesorter("+options+");") with JsMember
  
  def jsRender(selector: String): JsExp = jsRender(selector,CustomTableSorter.options())
